...
serve(async (req) => {
  ...
  try {
    ...
    // If this admin email already exists, return the existing corporate instead of failing.
    // This makes repeated submissions safe and avoids duplicate/mixup errors.
    const { data: existingAdmin } = await adminClient
      .from("corporate_admins")
      .select("id, corporate_id, email")
      .eq("email", normalizedEmail)
      .maybeSingle();

    if (existingAdmin) {
      const { data: existingCorporate, error: existingCorporateError } = await adminClient
        .from("corporates")
        .select("*")
        .eq("id", existingAdmin.corporate_id)
        .maybeSingle();

      if (existingCorporateError) {
        return new Response(JSON.stringify({ error: existingCorporateError.message }), {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
      ...
      return new Response(
        JSON.stringify({
          success: true,
          existing: true,
          message: "A corporate already exists for this admin email",
          corporate: existingCorporate,
        }),
        { headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
